...
Focus Radio сидит в меню-баре и стримит 18 отобранных станций для фонового прослушивания:
ambient, space, jazz, mellow, downtempo, vaporwave. Без аккаунтов, без рекламы, без
...
- **18 отобранных станций, четыре провайдера.** SomaFM (10 каналов: ambient / space / IDM /
  jazz), Radio Paradise (Mellow / Main / Global), NTS Mixtapes (Slow Focus / Low Key / Sheet
  Music / Expansions) и Nightwave Plaza (vaporwave / lo-fi). Каждая станция проверена на
  реальное воспроизведение, а не просто на коннект.
- **Индикатор здоровья станции.** Цветной кружок у строки статуса — зелёный (играет),
  жёлтый (подключение), красный (недоступна) — сразу видно, если поток не отвечает.
- **Двуязычие.** Английский и русский интерфейс, переключается автоматически по языку
  системы.
- **Проверка обновлений.** Из окна «О программе» — опциональная сверка с GitHub Releases,
  только уведомление, без фоновых загрузок.
- **Надёжный фоллбэк.** Watchdog по каждому URL → следующий URL → хардкодный снимок →
  повтор всей станции. Реальный факт воспроизведения — по байтам/`currentTime`,
...
аффилирован с SomaFM, Radio Paradise, NTS и Nightwave Plaza. Если вам нравится какая-то из
этих станций, поддержите её напрямую — они без рекламы и живут на пожертвования слушателей.
