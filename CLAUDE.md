...
- Используем инструмент Bash (не PowerShell). PowerShell-скрипты, оставшиеся в репо (`check.ps1`), — артефакты прежнего демо.
...
- **`check.sh` / `check.ps1`** — реликты предыдущей задачи (мониторинг диска), к текущему коду отношения не имеют.
...
- **Режимы запуска:** `--test-all` итерирует станции через цепочку `Timer`-ов, не блокируя главный run loop. Гонка с AVPlayer возникает, если попытаться драйвить run loop вручную через `RunLoop.main.run(until:)` — AVPlayer перестаёт обращаться по сети.
