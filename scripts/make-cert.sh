#!/usr/bin/env bash
# Создаёт локальный self-signed сертификат для подписи Focus Radio и кладёт его в
# login keychain. Нужен один раз. Стабильная подпись → выданные разрешения
# (микрофон/keychain и т.п.) держатся между запусками и обновлениями, в отличие
# от ad-hoc, где идентичность меняется на каждой сборке.
#
...
openssl pkcs12 -export -legacy -inkey "$TMP/key.pem" -in "$TMP/cert.pem" \
    -out "$TMP/cert.p12" -passout pass:focusradio -name "$IDENTITY" 2>/dev/null

# -A: ключ доступен приложениям без запроса (локальный dev-сертификат).
security import "$TMP/cert.p12" -k "$HOME/Library/Keychains/login.keychain-db" -P focusradio -A
